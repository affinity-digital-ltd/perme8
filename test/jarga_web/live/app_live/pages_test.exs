defmodule JargaWeb.AppLive.PagesTest do
  ...
  describe "page show (unauthenticated)" do
    test "redirects if user is not logged in", %{conn: conn} do
      assert {:error, redirect} =
               live(conn, ~p"/app/workspaces/test-workspace/documents/test-document")
  ...
  describe "page show (authenticated)" do
    ...
    test "allows updating page title with autosave on blur", %{
      ...
      lv
      |> element("input[name='document[title]']")
      |> render_blur(%{document: %{title: "Updated Title"}})
    ...
    test "allows toggling pin status", %{conn: conn, user: user, workspace: workspace} do
      ...
      updated_page = Documents.get_document!(user, page.id)
    ...
    test "does not allow access to private pages from other users", %{conn: conn} do
      ...
      {:ok, other_page} =
        Documents.create_document(other_user, other_workspace.id, %{
          title: "Private Page",
          is_public: false
        })

      {:error, {:redirect, %{to: "/app/workspaces"}}} =
        live(conn, ~p"/app/workspaces/#{other_workspace.slug}/documents/#{other_page.slug}")
    end
    ...
    test "allows workspace members to view public pages from other users", %{
      ...
      {:ok, public_page} =
        Documents.create_document(other_user, workspace.id, %{
          title: "Public Page",
          is_public: true
        })
      ...
      {:ok, _lv, html} =
        live(conn, ~p"/app/workspaces/#{workspace.slug}/documents/#{public_page.slug}")
    ...
    test "deletes page and redirects when delete button clicked", %{
      ...
      # Click delete button
      lv |> element("button[phx-click='delete_document']") |> render_click()

      # Should redirect to workspace
      assert_redirect(lv, ~p"/app/workspaces/#{workspace.slug}")

      # Verify page is deleted
      assert {:error, :document_not_found} = Documents.delete_document(user, page.id)
    end
  end
  ...
  describe "collaborative editing" do
    ...
    setup %{conn: conn} do
      user = user_fixture()
      workspace = workspace_fixture(user)
      {:ok, page} = Documents.create_document(user, workspace.id, %{title: "Collaborative Page"})
    ...
    test "debounces database saves on server side", %{
      ...
      JargaWeb.DocumentSaveDebouncer.wait_for_save(page.id)

      # Note should be updated with the final state
      page = Documents.get_document!(user, page.id, preload_components: true)
  ...
  describe "staleness detection" do
    setup %{conn: conn} do
      user = user_fixture()
      workspace = workspace_fixture(user)
      {:ok, page} = Documents.create_document(user, workspace.id, %{title: "Staleness Test Page"})
    ...
    test "handles get_current_yjs_state event", %{
      ...
      page_with_components = Documents.get_document!(user, page.id, preload_components: true)
      note = Documents.get_document_note(page_with_components)
