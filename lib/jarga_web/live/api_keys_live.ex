defmodule JargaWeb.ApiKeysLive do
  ...
  def mount(_params, _session, socket) do
    ...
    {:ok, all_keys} = Accounts.list_api_keys(current_user.id)
  ...
  def handle_event("create_key", %{"name" => name, "description" => description} = attrs, socket) do
    ...
    case Accounts.create_api_key(current_user(socket).id, api_key_attrs) do
      {:ok, {_api_key, plain_token}} ->
        # Reload API keys list
        {:ok, all_keys} = Accounts.list_api_keys(current_user(socket).id)
        active_keys = Enum.filter(all_keys, & &1.is_active)
        inactive_keys = Enum.filter(all_keys, &(!&1.is_active))
  ...
  def handle_event(
    ...
    case Accounts.update_api_key(current_user(socket).id, api_key_id, update_attrs) do
      {:ok, _updated_key} ->
        # Reload API keys list
        {:ok, all_keys} = Accounts.list_api_keys(current_user(socket).id)
        active_keys = Enum.filter(all_keys, & &1.is_active)
  ...
  def handle_event("revoke_key", %{"id" => api_key_id}, socket) do
    case Accounts.revoke_api_key(current_user(socket).id, api_key_id) do
      {:ok, _revoked_key} ->
        # Reload API keys list
        {:ok, all_keys} = Accounts.list_api_keys(current_user(socket).id)
        active_keys = Enum.filter(all_keys, & &1.is_active)
